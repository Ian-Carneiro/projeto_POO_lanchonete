...
public class Comanda {
	...
	int buscarPedido(int numeroPedido) {// busca um pedido pelo seu numero na comanda retornando o indice da lista(comanda)
		if(!comanda.isEmpty()) {
	...
	boolean removePedido(int numeroPedido) {
		if(comanda.remove(buscarPedido(numeroPedido))!=null) {
			return true;
		}
		return false;
	}
	float valorTotal() {//valor total da comanda
		float total = 0;
	...
	Pedido getPedido(int numeroPedido) {// retorna um pedido específico da comanda (relaciona esta classe com GerenciaComanda)
		return comanda.get(buscarPedido(numeroPedido));
	}
	List<Pedido> getListaPedidos(){// retorna uma lista de todos os pedidos da comanda (relaciona esta classe com GerenciaComanda)
		return comanda;
	}
	@Override
